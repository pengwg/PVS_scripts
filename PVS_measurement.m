...
parfor n = 1 : numSubjects
    subject = sprintf('PVS_%03d', n);
    pvs_mask_file = sprintf('%s/%s/PVS_%03d_vsmask_%s.nii.gz', PVS_path, subject, n, region);
    ...
    writetable(table(length, width, volume), sprintf('%s/%s/PVS1_%03d_%s.xlsx', PVS_path, subject, n, region), 'WriteMode', 'replacefile');
end
...
writetable(T, [PVS_path '/PVS1_stats_' region, '.xlsx'], 'WriteMode', 'replacefile')
